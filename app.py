...
QUERY_RECOMMENDATIONS_FOLLOWING = """
    SELECT P.*, SUM(value) v
    FROM posts P
             LEFT JOIN comments C ON P.id = C.post_id
             LEFT JOIN reactions R ON P.id = R.post_id,
         (SELECT DISTINCT IU.id uid, COUNT(DISTINCT IP.id) value
          ...
          WHERE IU.id != ?
            AND IP.id IN (SELECT reactions.post_id
                          FROM reactions
                          WHERE reactions.user_id = ?)
            AND uid IN (SELECT followed_id FROM follows WHERE follower_id = ?))
    WHERE C.user_id = uid
       OR R.user_id = uid
    GROUP BY P.id, P.created_at
    ORDER BY v DESC, P.created_at DESC
...
def recommend(user_id, filter_following):
    ...
    if filter_following:
        return query_db(QUERY_RECOMMENDATIONS_FOLLOWING, args=(user_id, user_id, user_id, user_id, user_id))
    else:
        return query_db(QUERY_RECOMMENDATIONS, args=(user_id, user_id, user_id, user_id))
